...
describe('previewGuardHooksHandler', () => {
    it('parses params and returns preview entries from the preview engine', async () => {
        const listHooksDetailed = vi.fn().mockResolvedValue({ hooks: [] });
        const components = {
            hooksManager: { listHooksDetailed, getCachedRuleContent: vi.fn() },
        ...
        expect(listHooksDetailed).toHaveBeenCalledTimes(1);
    });
